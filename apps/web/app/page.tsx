...
import { Shield, Lock, FileCheck, Share2, Check, Zap, Eye, Clock, ArrowRight, Fingerprint, ShieldCheck, Globe } from "lucide-react";
			...
			<nav className="fixed top-0 w-full z-50 bg-white/70 backdrop-blur-xl border-b border-slate-100">
				<div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
					<div className="flex items-center gap-3">
						...
						<span className="text-xl font-bold tracking-tight text-slate-950">MediVault</span>
					</div>
					...
					<div className="flex items-center gap-4">
						<Link href="/login">
							<Button variant="ghost" className="text-sm font-bold text-slate-600 hover:text-blue-600">
								Sign In
							</Button>
						</Link>
						<Link href="/signup">
			...
			<section className="relative pt-40 pb-20 overflow-hidden">
				...
				<div className="max-w-7xl mx-auto px-6 text-center space-y-10">
					...
					<p className="max-w-2xl mx-auto text-lg md:text-xl text-slate-500 font-medium leading-relaxed animate-in fade-in slide-in-from-bottom-8 duration-1000 delay-200">
						MediVault combines AWS hardware security with Hedera blockchain 
						to ensure your records are encrypted, immutable, and entirely yours.
					</p>
					...
					<div className="flex flex-col sm:flex-row justify-center gap-4 pt-6 animate-in fade-in slide-in-from-bottom-10 duration-1000 delay-300">
						<Link href="/signup">
							<Button size="lg" className="h-14 px-10 rounded-full text-base font-bold shadow-blue-200 shadow-2xl bg-blue-600 hover:bg-blue-700">
								Open Your Vault
								<ArrowRight className="ml-2 h-5 w-5" />
							</Button>
						</Link>
						<Link href="/signup">
							<Button size="lg" variant="outline" className="h-14 px-10 rounded-full text-base font-bold border-2 border-slate-100 bg-white hover:bg-slate-50">
								Watch Demo
							</Button>
						</Link>
					</div>
						...
						<div className="max-w-5xl mx-auto rounded-[40px] border-[12px] border-slate-900 bg-slate-900 shadow-2xl overflow-hidden aspect-[16/9]">
							...
							<div className="w-full h-full bg-[#f8fafc] p-8 flex flex-col items-start text-left space-y-8">
								<div className="w-full h-12 flex justify-between items-center border-b pb-4">
									...
									<div className="bg-slate-100 px-4 py-1 rounded-lg text-[10px] font-bold text-slate-400 uppercase tracking-widest">secure.medivault.io</div>
								</div>
				...
				<div className="max-w-7xl mx-auto px-6">
					<div className="grid grid-cols-2 md:grid-cols-4 gap-12 text-center">
						<div className="space-y-2">
							...
							<div className="text-xs font-bold text-slate-400 uppercase tracking-widest">KMS Encryption</div>
						</div>
						<div className="space-y-2">
							<div className="text-4xl font-black text-slate-900">Zero</div>
							<div className="text-xs font-bold text-slate-400 uppercase tracking-widest">Access Protocol</div>
						</div>
						<div className="space-y-2">
							<div className="text-4xl font-black text-slate-900">100%</div>
							<div className="text-xs font-bold text-slate-400 uppercase tracking-widest">Immutable Logs</div>
						</div>
						<div className="space-y-2">
							<div className="text-4xl font-black text-slate-900">Global</div>
							<div className="text-xs font-bold text-slate-400 uppercase tracking-widest">HIPAA Ready</div>
						</div>
					</div>
				</div>
			</section>
				...
				<div className="max-w-7xl mx-auto px-6">
					<div className="grid md:grid-cols-2 gap-20 items-center">
						<div className="space-y-8">
							<Badge variant="outline" className="rounded-full border-blue-200 text-blue-600 bg-blue-50/50 h-8 px-4 font-bold uppercase tracking-widest text-[10px]">Security Infrastructure</Badge>
							...
							<p className="text-lg text-slate-500 leading-relaxed font-medium">
								We believe your health data shouldn't be accessible by anyone but you. 
								Our architecture ensures that even we cannot read your files.
							</p>
						...
						<div className="bg-slate-50 rounded-[60px] aspect-square flex items-center justify-center relative overflow-hidden group">
							...
							<div className="absolute bottom-12 left-12 right-12 bg-white/80 backdrop-blur-xl border border-white p-8 rounded-3xl shadow-xl">
								<p className="text-sm font-bold text-slate-900 leading-relaxed italic">
									"The combination of AWS and Hedera creates a security layer that is virtually impossible to compromise."
								</p>
								<div className="mt-4 flex items-center gap-3">
									<div className="w-10 h-10 rounded-full bg-slate-900" />
									<div>
										<div className="text-xs font-bold text-slate-900">Dr. Sarah Chen</div>
										<div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Head of Cybersecurity</div>
									</div>
								</div>
							</div>
						</div>
				...
				<div className="max-w-7xl mx-auto px-6">
					<div className="grid md:grid-cols-4 gap-12 pb-20 border-b border-white/10">
						<div className="col-span-2 space-y-6">
							<div className="flex items-center gap-3">
								...
								<span className="text-2xl font-bold tracking-tight text-white">MediVault</span>
							</div>
							<p className="max-w-sm text-slate-400 font-medium leading-relaxed">
								The future of medical record storage. Secure, private, and 100% owned by the patient.
							</p>
						</div>
						<div className="space-y-6">
					...
					<div className="pt-12 flex flex-col md:flex-row justify-between items-center gap-6">
						<p className="text-sm font-medium text-slate-500 italic">
							© 2026 MediVault. All records are secured by AWS & Hedera Hashgraph.
						</p>
						<div className="flex items-center gap-6">
							<div className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] border border-slate-800 px-3 py-1 rounded-full">Secure Node: Testnet-4</div>
						</div>
					</div>
				</div>
			</footer>
	...
	return (
		<div className="flex items-start gap-5 group">
			<div className="shrink-0 w-12 h-12 rounded-2xl bg-blue-50 flex items-center justify-center group-hover:bg-blue-600 transition-colors duration-500">
				<span className="group-hover:text-white transition-colors duration-500">{icon}</span>
			</div>
			<div className="space-y-1 pt-1">
				<h3 className="font-bold text-slate-900 group-hover:text-blue-600 transition-colors duration-500">{title}</h3>
